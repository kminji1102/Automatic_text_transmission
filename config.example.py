# config.example.py — 템플릿입니다. 이 파일을 복사해 config.py로 만들고 실제 값을 채우세요.
#   Windows(PowerShell): Copy-Item config.example.py config.py
# config.py는 .gitignore에 등록되어 깃/깃허브에 올라가지 않습니다(발신번호·노션 ID 보호).
import os
...
NOTION_DATA_SOURCE_ID = "여기에_노션_데이터소스_ID_입력"
