...
기수 매핑은 포함 방식입니다. 예를 들어 노션 기수 값이 `SFAC 33기`, `플레이데이터 33기 수강생`처럼 들어와도 `"33기"` 키가 포함되어 있으면 해당 발신번호로 매핑됩니다.
...
- 신규 기수나 발신번호는 `config.py`의 `COHORT_SENDER_MAP`에만 추가합니다.
